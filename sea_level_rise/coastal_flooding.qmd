...
1st test: Does intersection select the whole district or create new polygons that align with the flooding layer. The test involves picking one school district affected by sea level rise, cropping the sea level rise polygon to it, and then plotting the two on top of each other
```{r}
# find the intersections
#slr_districts <- st_intersection(ca_districts, slr_schools_test)
plot(SA)
...
#SA <- slr_districts %>% 
 # filter(DistrictNa == "Santa Ana Unified")

SA_flood <- st_crop(slr_schools_test, st_bbox(SA))
