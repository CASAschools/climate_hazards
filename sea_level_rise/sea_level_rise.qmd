...
leaflet() %>% 
  ...
  addPolygons(data = dp_slr_clip, fillColor = "blue", group = "area1") %>% 
  
  # add overlap sea level rise polygons
  addPolygons(data = dp_slr_poly, fillColor = "red", opacity = .7, group = "area2") %>% 
  ...
  # add option to toggle data on and off
   addLayersControl(
    overlayGroups = c("area1", "area2", "school area", "school point"),
    options = layersControlOptions(collapsed = FALSE)  # Keeps the layers control open
  ) %>% 
  
  # set zoom to be the bounding box of the school buffer
  setView(leaflet::bbox(dp))
```
