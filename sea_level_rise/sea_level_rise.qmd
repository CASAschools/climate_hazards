...
ca_slr_files <- list.files("/capstone/casaschools/sea_level_rise/raw_data/cosmos/all", pattern = "*.shp", full.names = TRUE)

# read in files and transform to the crs of the california schools buffers
read_transform_crs <- function(ca_slr_files, target_crs = 3857) {
  
  # read in shapefile
  shapefile <- st_read(ca_slr_files, quiet = TRUE)
...
ca_schools_slr <- st_intersection(ca_schools_clip, ca_slr)
...
# select the sea level rise polygons that overlap
dp_slr_poly <- ca_slr[dp, ]

# select sea level rise polygons that intersect
dp_slr_clip <- st_intersection(dp, dp_slr_poly)
...
leaflet() %>% 
  ...
  addPolygons(data = dp_slr_clip, fillColor = "blue",
              fillOpacity = 1, stroke = FALSE, group = "school sea level rise extent") %>% 
  
  # add overlap sea level rise polygons
  addPolygons(data = dp_slr_poly, fillColor = "seagreen", 
              fillOpacity = .8, stroke = FALSE, group = "county sea level rise extent") %>% 
  
  # add school buffer polygon
  addPolygons(data = dp, color = "black", fill = FALSE,
              weight = 2, group = "school area") %>% 
  ...
  addLayersControl(
    overlayGroups = c("school sea level rise extent", "county sea level rise extent", 
                      "school area", "school point"),
    options = layersControlOptions(collapsed = FALSE)) %>% 
  
  # set zoom to be the center of the school
  setView(lng = dp_point$Longitude, lat = dp_point$Latitude, zoom = 12)
```
