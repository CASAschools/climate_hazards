...
data_dir_precip <- tools::R_user_dir("caladaptr_pr", which = "data")
schools_dir_precip <- file.path(data_dir_precip, "schools") %>% normalizePath(mustWork = FALSE)
...
locaschl_et_rtbl_1 <- locaschl_et_rtbl_1 %>%
  mutate(threshold = ifelse(val > 18.671, 1, 0)) %>%
...
# Export file
write.csv(ca_schools, "/capstone/casaschools/output_data/ca_schools_35_64_precip.csv")

#write.csv(ca_schools, "/github/climate_hazards/output_data/ca_schools_35_64_precip.csv")
```